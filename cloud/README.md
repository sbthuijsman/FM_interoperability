# README
### Launching (provider, consumer) pair

0. Setup local arrowhead cloud core systems (https://arrowhead.eu/why-how/what-is-it/how-to-implement/)
1. Update application.properties (in src) for provider and consumer for current IP and select unique port
2. Build with maven install
3. Run -java jar for provider
4. Register consumer by (using swagger) POST serviceregistry/register, something along the lines of: (change ip, port, and consumer name)
```
{
"endOfValidity": "2024-03-10T11:33:27Z",
"interfaces": [
"HTTP-INSECURE-JSON"
],
"metadata": {
...
"port": xxx,
"systemName": "LSAT-CONSUMER"
},
"secure": "NOT_SECURE",
"serviceDefinition": "lsat-consumer",
"serviceUri": "/CONSUMER",
"version": 0
}
```
5. Run the authorization powershell (.ps) to write authorization rules (one time)
6. run -java jar for consumer
